...
++++
\begin{align*}
M_{90^{\circ}}&=\left[ \begin{array}{} 0 & 0 & -1} \\ \sin{R}\cos{Y}-\cos{R}\sin{Y} & \sin{R}\sin{Y}+\cos{R}\cos{Y} & 0 \\ \cos{R}\cos{Y}+\sin{R}\sin{Y} & \cos{R}\sin{Y}-\sin{R}\cos{Y} & 0 \\ \end{array} \right]
\end{align*}
++++

The formula we use to calculate yaw and roll becomes stem:[\mathrm{atan2}(0,0)], which will give an invalid value.
