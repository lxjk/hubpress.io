...
[stem]
++++
M=\bigl(\begin{matrix} a\vec{X} & 0 \\ b\vec{Y} & 0 \\ c\vec{Z} & 0 \\ \vec{T} & 1 \\ \end{matrix}\bigr)= \bigl(\begin{matrix} aX_0 & aX_1 & aX_2 & 0 \\ bY_0 & bY_1 & bY_2 & 0 \\ cZ_0 & cZ_1 & cZ_2 & 0 \\ T_0 & T_1 & T_2 & 1 \\ \end{matrix}\bigr)
++++
...
[stem]
++++
M^{-1}=\bigl(\begin{matrix} \vec{X} & \vec{Y} & \vec{Z} & \vec{0} \\ -\vec{T}\cdot\vec{X} & -\vec{T}\cdot\vec{Y} & -\vec{T}\cdot\vec{Z} & 1 \\ \end{matrix}\bigr)
++++
...
[stem]
++++
M=\bigl(\begin{matrix} A & B \\ C & D \\ \end{matrix}\bigr)=\bigl(\begin{matrix} A_0 & A_1 & B_0 & B_1 \\ A_2 & A_3 & B_2 & B_3 \\ C_0 & C_1 & D_0 & D_1 \\ C_2 & C_3 & D_2 & D_3 \\ \end{matrix}\bigr)
++++
...
++++
\begin{align*}
{\bigl(\begin{matrix} A & B \\ C & D \\ \end{matrix}\bigr)}^{-1}&=\bigl(\begin{matrix} A^{-1}+A^{-1}B(D-CA^{-1}B)^{-1}CA^{-1} & -A^{-1}B(D-CA^{-1}B)^{-1} \\ -(D-CA^{-1}B)^{-1}CA^{-1} & (D-CA^{-1}B)^{-1} \\ \end{matrix}\bigr)\\
&=\bigl(\begin{matrix} (A-BD^{-1}C)^{-1} & -(A-BD^{-1}C)^{-1}BD^{-1} \\ -D^{-1}C(A-BD^{-1}C)^{-1} & D^{-1}+D^{-1}C(A-BD^{-1}C)^{-1}BD^{-1} \\ \end{matrix}\bigr)
\end{align*}
++++
...
[stem]
++++
{\bigl(\begin{matrix} A & B \\ C & D \\ \end{matrix}\bigr)}^{-1}=\bigl(\begin{matrix} (A-BD^{-1}C)^{-1} & -(A-BD^{-1}C)^{-1}BD^{-1} \\ -(D-CA^{-1}B)^{-1}CA^{-1} & (D-CA^{-1}B)^{-1} \\ \end{matrix}\bigr)
++++
...
[stem]
++++
A^{\#}={\bigl(\begin{matrix} A_0 & A_1 \\ A_2 & A_3 \\ \end{matrix}\bigr)}^{\#}=\bigl(\begin{matrix} A_3 & -A_1 \\ -A_2 & A_0 \\ \end{matrix}\bigr)
++++
...
Finally for our block matrices stem:[M={\bigl(\begin{matrix} A & B \\ C & D \\ \end{matrix}\bigr)}], the determinant is
...
Let stem:[M^{-1}={\bigl(\begin{matrix} A & B \\ C & D \\ \end{matrix}\bigr)}^{-1}={\bigl(\begin{matrix} X & Y \\ Z & W \\ \end{matrix}\bigr)}].Let’s start with the top left corner.
...
[stem]
++++
M^{-1}={\bigl(\begin{matrix} A & B \\ C & D \\ \end{matrix}\bigr)}^{-1}=\frac{1}{\left|M\right|}{\begin{matrix} \begin{array}{} (\left|D\right|A-B(D^{\#}C))^{\#} & (\left|B\right|C-D(A^{\#}B)^{\#})^{\#} \\ (\left|C\right|B-A(D^{\#}C)^{\#})^{\#} & (\left|A\right|D-C(A^{\#}B))^{\#} \\ \end{array} \right]}
++++
